...
<nav class="navbar navbar-expand-sm navbar-dark bg-dark mb-2">
    ...
    <form class="form-inline" action="" method="POST">
        <div class="card">
        	<div class="card-body ">
			    <h3 class="card-title">SALARY DEDUCTION DETAILS</h3>
                <hr><br>
                    <div class="text-center col-xs-4">
                        <input type="text" name="employee_id" class="form-control  mr-4" placeholder="Enter Employee id" required>  
                        <input type="text" name="working_days" class="form-control mr-4" placeholder="No. of Working Days" required>
						<button  type="submit" class="btn btn-outline-success btn-lg" name="go">Go</button><br><br> 
                            ...
                            <?php
                                ...
                                if(isset($_POST['go'])){
									...
									$query = "SELECT * FROM employee JOIN salary,emp_leave where employee.id = '$employee_id' AND employee.id = salary.id AND employee.id = emp_leave.id";
                                            ...
                                            if($result->num_rows > 0){
                                                ...
                                                while($row = $result->fetch_assoc()){
                                                   ...
                                                   <?php 
													...
													$deduction = $ctc/$working_days * $unpaid_leaves;

                                                    echo '<tr><td>'.$id.'</td>'.'<td>'.$name.'</td>'.'<td>'.$unpaid_leaves .'</td>'.
														'<td>'.$ctc.'</td>'.'<td>'.$deduction.'</td></<tr>';
                                                }
                                            }else{	
                                                    echo "No Data Found.";
                                                }
